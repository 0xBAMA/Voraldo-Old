...
void Voraldo_Draw::draw_ellipsoid(vec center_point, vec radii, unsigned char set, bool draw, bool mask)
{
 vec index;
 for(int i = 0; i < parent->x_dim; i++)
 	{
...
void Voraldo_Draw::draw_cylinder(vec bvec, vec tvec, double radius,unsigned char set, bool draw, bool mask)
{
...
void Voraldo_Draw::draw_tube(vec bvec, vec tvec, double inner_radius, double outer_radius,unsigned char set, bool draw, bool mask)
{
 vec ndirection = tvec - bvec;
